...
Two real bugs turned up once `v0.1.0`/`v0.1.1` actually shipped: `docker/metadata-action`'s `type=semver` with a plain `{{version}}` pattern strips the leading `v`, so the first publish pushed `0.1.1`/`0.1`/`latest` instead of `v0.1.1`/`v0.1` — meaning `docker pull ...:v0.1.1`, the exact string shown on the GitHub Releases page, 404ed. Fixed by pinning the pattern to `v{{version}}`/`v{{major}}.{{minor}}` so image tags always match the git release tag exactly, verified by re-checking GHCR's registry API directly after `v0.1.2` (no Docker daemon in this sandbox, so verification here means hitting `https://ghcr.io/v2/.../manifests/<tag>` with an anonymous pull token and confirming `200`, not an actual `docker pull`). Separately, the PR that first merged this branch into `main` was opened from a stale local ref one commit behind the actual branch tip, silently dropping `publish.yml` itself off of `main` — caught by checking `GET /repos/.../actions/workflows` and seeing only `CI` listed, no `Publish images`. Both required a second small PR to land the missing/fixed commit.

## 32. Post-launch pass: repo rename, fresh screenshots, stale landing-page copy

Once `v0.1.x` was actually live and public, three things had gone stale without anyone touching them on purpose: the repo itself was renamed `alldb` → `wharf` mid-session (the user renamed it on GitHub directly), the product's screenshots predated the SaaS token-rewrite redesign (§27) and the Data-tab split (§29), and a chunk of marketing copy on the landing page still described the pre-split two-view (Simple/Advanced) product instead of the current three-tab one (Simple/Data/Advanced).

**Every `github.com/drk1rd/alldb` reference repointed to `drk1rd/wharf`** — the CI badge and repo links in `README.md`, all eight occurrences across `landing/index.html` (nav, hero, footer, quickstart CTA), `package.json`'s `repository`/`bugs`/`homepage` fields, and `.github/ISSUE_TEMPLATE/config.yml`'s security-advisory link. GitHub's redirect from the old name would have kept all of these technically working, but a public-facing repo pointing at its own former name via a redirect chain isn't a good look for something asking to be taken seriously.

**Fresh screenshots, taken the same way as every other UI verification pass this session** — Playwright against the real dev server with `page.route()` standing in for a populated account (5 running instances across all engines, a `users` table with real-looking rows) since this sandbox has no Docker daemon to create actual instances. Replaced `docs/screenshots/dashboard.png` and renamed `instance-simple.png` → `instance-data.png`, since the more compelling and more accurate shot of "what makes this different from Adminer" is the Data tab now, not the trimmed-down Simple view. Same two files copied into `landing/assets/` per that directory's own stated convention (`landing/README.md`: keep both copies in sync).

**Landing-page copy fixes**: the quickstart's copy-paste command was still `cd deploy && docker compose up --build` — stale since §31 made pulling pre-built images the default and removed the need for `--build` entirely; anyone who ran the old command would still get a from-source build, silently missing the point of the whole feature. The "duality" section's eyebrow/heading/body copy hard-coded a "one instance, two views" narrative ("hides everything except connect and browse... raw config, live metrics, logs, and a shell") that predates the Data-tab split — rewritten to describe the actual three tabs (Simple: connect + ask; Data: browse/filter/edit/query; Advanced: config/metrics/logs/shell) without hard-coding a specific count that'll go stale again the next time a tab gets added. The logo's wavy-water squiggle path — removed from the product UI's own `Logo.tsx` in §28 for reading too "kid-like" — was still present in all three of the landing page's own logo instances (favicon data-URI, nav, footer), since that fix only ever touched `web/src/components/Logo.tsx`. Removed from all three so the mark is consistent between the marketing site and the product, even though the two intentionally keep different overall visual identities otherwise.

**Verified live**: served `landing/` locally (`python3 -m http.server`) and screenshotted the hero, quickstart, and duality sections with Playwright post-fix — confirmed both logo instances render as just deck+piers with no dangling SVG, the quickstart code block reads `docker compose up` with no `--build`, and the duality section's two screenshots load correctly with copy that matches what's actually in them.
